...
namespace WindowsToolService
{
    internal sealed class DesktopTools
    {
        internal static void RequireInteractiveDesktop()
        {
            ...
            if (desktop == IntPtr.Zero) throw new InvalidOperationException("Desktop unavailable or locked. Unlock the signed-in session first.");
            try
            {
                if (!SwitchDesktop(desktop)) throw new InvalidOperationException("Input is unavailable on a locked or secure desktop.");
            }
            finally { CloseDesktop(desktop); }
        }

        internal object Call(string tool, IDictionary<string, object> args)
        {
            RequireInteractiveDesktop();
            switch (tool)
            {
                case "mouse.move": return Move(args, false);
                case "mouse.click": return Move(args, true);
                case "keyboard.typeText":
                    ...
                    return Result();
                case "keyboard.keyPress": return Press(args);
                case "screenshot.capturePrimaryDisplay": return Capture();
                case "window.listWindows": return Windows();
                default: throw new ArgumentException("Unsupported tool: " + tool);
            }
        }

        private static Dictionary<string, object> Result()
        {
            return new Dictionary<string, object> { { "success", true }, { "backend", "win32" }, { "timestamp", DateTime.UtcNow.ToString("o") } };
        }

        private static object Move(IDictionary<string, object> args, bool click)
        {
            var bounds = SystemInformation.VirtualScreen;
            var positionX = Arguments.Integer(args, "x", bounds.Left, bounds.Right - 1);
            var positionY = Arguments.Integer(args, "y", bounds.Top, bounds.Bottom - 1);
            ...
            if (!SetCursorPos(positionX, positionY)) throw new Win32Exception();
            if (click)
            {
                var down = button == "right" ? 0x0008u : 0x0002u;
                for (var count = 0; count < (clickType == "double" ? 2 : 1); count++)
        ...
        {
            object raw;
            if (!args.TryGetValue("keys", out raw) || !(raw is IList)) throw new ArgumentException("keys must be an array.");
            var names = (IList)raw;
            if (names.Count == 0 || names.Count > 16) throw new ArgumentException("Provide between 1 and 16 keys.");
            ...
            finally
            {
                ...
                for (var index = pressed.Count - 1; index >= 0; index--)
                    releases.Add(KeyInput(pressed[index], 0, Extended(pressed[index]) | 2));
        ...
        internal static ushort ResolveKey(string name)
        {
            ...
            ushort key;
            if (!KeyNames.TryGetValue(name, out key)) throw new ArgumentException("Unsupported key name: " + name);
            return key;
        }
        ...
        private static object Windows()
        {
            ...
            EnumWindow callback = delegate(IntPtr handle, IntPtr parameter)
            {
                ...
                if (title.Length > 0 && GetWindowRect(handle, out bounds))
                    windows.Add(new { title = title.ToString(), x = bounds.Left, y = bounds.Top,
                        width = bounds.Right - bounds.Left, height = bounds.Bottom - bounds.Top, isFocused = handle == foreground });
                return true;
            };
        ...
        [DllImport("user32.dll", SetLastError = true)] private static extern bool SetCursorPos(int positionX, int positionY);
