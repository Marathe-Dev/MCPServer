...
| `screenshot.capture` | 60,000 ms (encoding + upload can be slow) |
...
Capture a screen region. **Requires storage to be configured on the device** (§4.11) — there is no inline-bytes fallback, so a misconfigured device fails clearly instead of flooding the relay with image bytes.
...
```
> Map a screenshot pixel back to a real screen coordinate as `originX + pixelX / scale`. There is never a `base64Data` field — image bytes never travel over the relay/broker, only the presigned URL. If storage isn't configured, you'll get `ok:false, error:"Storage is not configured on this device. Configure storage to use screenshot.capture."`
