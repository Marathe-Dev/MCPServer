...
for (let i = 1; i <= 24; i++) {
  KEY_NAME_MAP[`f${i}`] = Key[`F${i}` as keyof typeof Key];
}
for (let d = 0; d <= 9; d++) {
  KEY_NAME_MAP[`${d}`] = Key[`Num${d}` as keyof typeof Key];
}
for (const letter of "abcdefghijklmnopqrstuvwxyz") {
  KEY_NAME_MAP[letter] = Key[letter.toUpperCase() as keyof typeof Key];
}
 ...
 */
export class NutjsKeyboardService implements IKeyboardService {
  async typeText(input: TypeTextInput): Promise<KeyboardActionResult> {
  ...
  async keyPress(input: KeyPressInput): Promise<KeyboardActionResult> {
