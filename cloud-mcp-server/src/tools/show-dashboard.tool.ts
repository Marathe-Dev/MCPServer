...
type DashboardSnapshot = z.infer<typeof snapshotSchema>;

function snapshot(registry: DeviceRegistry): DashboardSnapshot {
...
export function registerShowDashboardTool(
  ...
  server.registerResource(
    "remotepc-dashboard-ui",
    DASHBOARD_RESOURCE_URI,
    { title: "RemotePC Dashboard", mimeType: RESOURCE_MIME },
    async () => ({
      contents: [
        {
  ...
  server.registerTool(
    ...
    {
      description:
        "Show the connected-devices dashboard as an interactive inline widget. Call this whenever the user asks to see, open, or get the devices dashboard.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Return current connected-device data for the dashboard widget (used by the dashboard UI to refresh).",
      inputSchema: z.object({}),
      outputSchema: snapshotSchema,
