...
export function registerGetFileTool(server: McpServer, deviceRegistry: DeviceRegistry): void {
  server.registerTool(
    ...
    {
      description:
        "Read a file from the Windows tool service device. Uploads it to storage and returns a presigned download URL (no base64). Absolute Windows path required. Files larger than 10 MB are rejected.",
