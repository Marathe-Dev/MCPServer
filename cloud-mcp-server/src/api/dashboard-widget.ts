...
/** Inline SVG platform icons (server-side copy; the widget script has its own for re-renders). */
const PLATFORM_ICON: Record<string, string> = {
  win32:
    '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M3 5.5 10 4.5v7H3zM10 12.5v7L3 18.5v-6zM11 4.3 21 3v8.5H11zM21 12.5V21l-10-1.3v-7.2z"/></svg>',
  darwin:
    '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M16 3c.1 1.2-.4 2.3-1.1 3.1-.7.9-1.9 1.5-3 1.4-.1-1.1.4-2.3 1.1-3 .8-.9 2-1.5 3-1.5zM19.5 17.4c-.5 1.2-.8 1.7-1.4 2.7-.9 1.4-2.2 3.2-3.8 3.2-1.4 0-1.8-.9-3.7-.9s-2.3.9-3.7.9c-1.6 0-2.8-1.6-3.7-3C.9 17.1.6 12.3 2.6 9.9c1-1.2 2.4-1.9 3.8-1.9 1.5 0 2.4.9 3.7.9 1.2 0 2-.9 3.7-.9 1.3 0 2.6.7 3.6 1.9-3.1 1.7-2.6 6.2.1 7.6z"/></svg>',
  linux:
    '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M12 2c2 0 3 2 3 4 0 1 .5 2 1 3 1 1.5 2 3 2 5 0 1-.5 2-1 3 .5 1 1 2 0 3-1 .5-2 0-3-.5-.5.5-1.5 1-3 1s-2.5-.5-3-1c-1 .5-2 1-3 .5-1-1-.5-2 0-3-.5-1-1-2-1-3 0-2 1-3.5 2-5 .5-1 1-2 1-3 0-2 1-4 3-4z"/></svg>',
  unknown:
    '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M4 5h16v11H4zM2 19h20v2H2z"/></svg>',
};
...
function iconFor(platform: string): string {
  return PLATFORM_ICON[platform] ?? PLATFORM_ICON.unknown;
}
...
function cardHtml(d: DeviceInfo): string {
  const on = d.status === "online";
  const dis = on ? "" : "disabled";
  const id = esc(d.deviceId);
  return (
    '<div class="card">' +
    `<div class="card__top"><div class="card__name">${iconFor(d.platform)}<span title="${esc(d.deviceName)}">${esc(d.deviceName)}</span></div>` +
    `<span class="badge ${on ? "on" : "off"}">${esc(d.status)}</span></div>` +
    `<div class="card__meta">${esc(d.platform)} · last active ${relTime(d.lastActive)}</div>` +
    '<div class="card__actions">' +
    `<button class="btn" ${dis} data-tool="screenshot" data-device="${id}">Screenshot</button>` +
    `<button class="btn" ${dis} data-tool="get_window_list" data-device="${id}">Windows</button>` +
    "</div>" +
    '<div class="card__form">' +
    `<div class="row"><input type="text" placeholder="Text to type…" data-type-input="${id}" ${dis}/><button class="btn" ${dis} data-type-send="${id}">Type</button></div>` +
    `<div class="row"><input type="number" placeholder="X" data-x="${id}" ${dis}/><input type="number" placeholder="Y" data-y="${id}" ${dis}/><button class="btn" ${dis} data-click="${id}">Click</button></div>` +
    "</div></div>"
  );
}

function rowHtml(d: DeviceInfo): string {
  const on = d.status === "online";
  return `<tr><td>${esc(d.deviceName)}</td><td><span class="badge ${on ? "on" : "off"}">${esc(d.status)}</span></td><td>${relTime(d.lastActive)}</td></tr>`;
}

/**
 * Builds the MCP Apps "View" (the dashboard widget HTML) served as a `ui://`
 * resource. The device cards/rows are **server-rendered into the HTML body**
 * (not just via the script) so the dashboard is visible with pure HTML+CSS —
 * even if a host's sandbox CSP blocks/breaks the inline script. The script
 * then only *enhances*: wires up buttons and refreshes live data via the
...
export function buildDashboardWidget(initialDevices: DeviceInfo[]): string {
  ...
  const onlineCount = initialDevices.filter((d) => d.status === "online").length;
  const countLabel = `${onlineCount}/${initialDevices.length} online`;
  const cards = initialDevices.length
    ? initialDevices.map(cardHtml).join("")
    : '<p class="empty">No devices connected. Start a local-tool-service.</p>';
  const rows = initialDevices.length
    ? initialDevices.map(rowHtml).join("")
...
<body>
  ...
  <div class="panel"><h2>Connected Devices</h2><div class="grid" id="grid">${cards}</div></div>
  <div class="panel"><h2>System Monitor</h2>
    <table><thead><tr><th>Device</th><th>Status</th><th>Last Active</th></tr></thead><tbody id="rows">${rows}</tbody></table>
  </div>
